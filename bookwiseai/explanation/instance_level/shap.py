...
def main(
    ...
    logger.info("Running instance-level explanation of Shapley method...")
    ...
    logger.info("Generating Shapley explanation for instance...")
    shap_instance = exp.predict_parts(
        instance,
        type="shap"
    )
    logger.info(f"\n{shap_instance.result}")
    shap_instance.plot(show=True)

    fig = shap_instance.plot(show=False)
    OUT_path = plot_path / "shap.png"
    fig.write_image(OUT_path)
    logger.info(f"Saved Shapley plot to: {OUT_path}")
